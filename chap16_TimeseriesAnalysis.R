# chap16_TimeseriesAnalysis

##################################################
##  Chapter16. 시계열 분석(Timeseries Analysis)       
##################################################
...
par(mfrow=c(1,3))
log<-diff(ts.plot(AirPassengers)) # ts.plot(시계열 시각화) - 비정상성 시계열 데이터
plot(log)

log <- diff(log(AirPassengers)) # 로그+차분 수행   - # diff : 차분시킴 
plot(log) # 분산 정상화 
...
plot(cbind(EuStock$DAX[1:1000], EuStock$SMI[1:1000]), main="주가지수 추세선")
##########################
# 시계열요소분해 시각화  #
##########################
...
# 자기 상관계수: 시계열 자료에서 시차(lag)를 일정하게 주는 경우 얻어지는 상관 계수.
...
# (2)Box-Ljung에 의한 잔차항 모형 진단
Box.test(model$residuals, lag=1, type = "Ljung") # p-value = 0.9879
