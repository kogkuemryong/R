...
# 가설 설정
# -연구가설(H1):기존 2014년도 고객불만율과 2015년도 CS교육 후 불만율에 차이가 있다.
# -귀무가설(H0):기존 2014년도 고객불만율과 2015년도 CS교육 후 불만율에 차이가 없다.
...
# p-value = 0.0006735 # 0.05 보다 작다 -> 귀무가설 기각, 연구가설 채택 
# # 해설: 기존만족율(80%)과 차이가 있다. -> 연구가설채택

binom.test(c(136, 14), p=0.8, alternative = "two.sided", conf.level = 0.95)
# conf.level : 신뢰도 ex) 95% -> 0.95 
...
# 파란줄 : 정규분포도를 따랐을 때 -  분포도 / 정규분포도를 따르고 있음을 시각적으로 확인 할 수 있다. 


# 단계6. 평균차이 검정 
# T-test(T-검정) : 모집단에서 추출한 표본 데이터의 분포형태가 정규분포일 때 수행.
# 1. 양측검정: x1 객체와 기존 모집단의 평균 5.2시간 비교 
t.test(x1, mu=5.2) # p-value = 0.0001417 # mu= "기준치 값" - 연구가설 채택 
...
# 단계1. 양측 검정
prop.test(c(110,135), c(150,150)) # PT/Coding 교육 방법에 대한 비율 차이 검정.(만족도를 넣음)
...
# p-value = 0.0001711 pt 만족도가 코딩 만족도 보다 떨어진다. 
...
# (6) 사후검정 시각화 
plot(TukeyHSD (result)) # 해석 ) A B C 집단간 모두 차이가 있다
