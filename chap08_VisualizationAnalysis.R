...
range(Chem97$age)
# [1] -6  5 / -6(1월생)~5(12월생) , 0(7월생)

# 2.1 히스토그램
# -형식) histogram(y축컬럼~ x축 컬럼|조건, data...)

histogram(~gcsescore, data = Chem97)
# gcsescore 변수를 대상으로 적용 히스토그램. 
...
# factor : 범주 값을 반환해준다. / score 요인 단위 


# 2.2 밀도 그래 -형식) densityp프 
#lot(~축컬럼|조건, data, groups=변수) 
...
#  plot.points=F 산점도 출력 x /  auto.key = T 범례 출력 O  
...
class(df) # data.frame생성

# (2) 막대그래프 그리기 = 데이터 형식 변경(matrix 형식을 table 형식으로 변경)

dfs <- as.data.frame.table(VADeaths)

str(dfs) # 20 obs. of  3 variables:
class(dfs) # "data.frame"
dfs #  Var1 Var2 Freq -> 1열 기준으로 data.table 생성
# 변형을 통해서 데이터셋의 의미를 분석, 특징을 직관적으로 파악하기 위함이다. 

# 막대그래프 그리기 
barchart(Var1~Freq|Var2, data=dfs)
barchart(Var1~Freq|Var2, data=dfs, layout=c(4,1))
# 율), Var2 변수 단위(그룹화:시골과 도시 출신의 남녀)로 x축 Freq(사망비
# y축 - Var1(사망연령대)으로 막대차트 프롤팅. 
# layout=c(4,1)속성은 4개의 패널을 1행에  나타내주는 역할을 제공 
...
# type="o" : 점(point) 타입으로 실선이 통과하는 유형으로 그래프의 타입 지정


# auto.key = list (배치위치, 점 추가, 선 추가) : 범례를 나타내는 속성으로 위치를 그래프에서 나타내고, 점과 선을 추가. 
...
# (2) 두 개 변수 대상 qplot() 함수 적용.  - displ(배기량)
...
qplot(clarity, data=diamonds, geom="freqpoly", group=cut, color=cut)
...
ggplot(data=mpg, aes(x=displ, y=hwy ,color=drv)) #aesthetics(미학)
...
# 단계 3(layer 3) : 축범위 조정하는 설정 추가하기. 
# x축 범위값을 3~6으로 지정
ggplot(data=mpg, aes(x=displ, y=hwy ,color=drv)) + geom_point() + xlim(3,6) + ylim(10,30)
