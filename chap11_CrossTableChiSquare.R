...
# 교차테이블에 카이검정 적용
CrossTable(x, y, chisq = T) # chisq = T : 유의 확률값으로 계산하여 결과를 도출. 
...
#  (1) 적합성 검정
#-------------------------
# 귀무가설(영가설): 기대치와 관찰치는 차이가 없다(관련성x). : p >= 알파 
#     예) 주사위는 게임에 적합하다.

# 연구가설(대립가설): 기대치와 관찰치는 차이가 있다(관련성o). : p < 알파 
#     예) 주사위는 게임에 적합하지 않다.
...
data <- textConnection( # textConnection : text 형태로 받아서 저장해준다. 
  "스포츠음료종류  관측도수 
      1               41
      2               30
      3               51
...
# 1. 파일 가져오기
data <- read.csv("C:/workspaces/R/data/homogenity.csv", header = T)
View(data)
...
length(data$survey) # [1] 150 (na 값이 없었음)
